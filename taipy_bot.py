...
def lstm_tahmin_yap(df, lookback_days=60):
    ...
    # Modeli eğitme (Günlük analiz sürecini çok yavaşlatmaması için epochs=10 olarak optimize edildi)
    model.fit(X_train, y_train, batch_size=16, epochs=10, verbose=0)
    
    # ---------------------------------------------------------
    # 6. GELECEK TAHMİNİ (Ertesi Gün İçin)
    # ---------------------------------------------------------
    # Son 60 günlük veriyi alıp yarının fiyatını tahmin ediyoruz
    son_pencere = scaled_X[-lookback_days:]
    son_pencere = np.reshape(son_pencere, (1, son_pencere.shape[0], son_pencere.shape[1]))
    
    tahmin_scaled = model.predict(son_pencere, verbose=0)
    gercek_tahmin = scaler_y.inverse_transform(tahmin_scaled)[0][0]
    
    return float(gercek_tahmin)
...
import yfinance as yf
import pandas as pd
# ... (Diğer tüm importların aynı kalacak)

# st.cache_resource yerine:
...
# st.cache_data yerine:
@functools.lru_cache(maxsize=32)
def veri_yukle(ticker, start, end, interval="1d", kaynak="Yahoo Finance (yfinance)"):
    # ... (İçerik tamamen aynı)
# --- TAIPY İÇİN ARAYÜZ DEĞİŞKENLERİ ---
    secilen_hisse = "THYAO"
    analiz_sonucu = "Henüz analiz yapılmadı."
    ai_hedef = 0.0

# Butona tıklandığında çalışacak ana fonksiyon
def analiz_baslat(state: State):
    # state üzerinden arayüzdeki değişkenlere erişiyoruz
    notify(state, 'info', f"{state.secilen_hisse} için analiz başlatılıyor...")
    
    # Kendi yazdığın asenkron veya normal analiz fonksiyonlarını çağırıyorsun
    # Örnek: veri = asenkron_analiz_yap(state.secilen_hisse, ...)
    
    # Analiz bittikten sonra sonuçları arayüze yansıt
    state.analiz_sonucu = f"{state.secilen_hisse} için günlük T3: BOĞA, Hacim: Normal"
    state.ai_hedef = 345.50 # Örnek sonuç
    
    notify(state, 'success', 'Analiz Tamamlandı!')

# --- TAIPY MARKDOWN ARAYÜZÜ (GUI) ---
# Streamlit'teki st.sidebar ve st.columns mantığını burada Markdown ile kuruyoruz.
sayfa_tasarimi = """
# 🚀 God Mode Terminal v100
...
<|Analiz Et|button|on_action=analiz_baslat|>
|>
...
**Yapay Zeka (XGBoost/LSTM) Hedefi:** <|{ai_hedef}|text|> TL
|>
|>
"""
...
if __name__ == "__main__":
    ...
    # Taipy GUI'yi çalıştır
    gui = Gui(page=sayfa_tasarimi)
    gui.run(title="God Mode Terminal", port=5000, dark_mode=True)
from taipy.gui import Gui, State
import pandas as pd
import numpy as np

# -------------------------------------------------------------------
# 1. YARDIMCI ANALİZ VE TAHMİN FONKSİYONLARI
# -------------------------------------------------------------------

def veri_getir(hisse_kodu: str):
    """BIST verisini getiren yardımcı fonksiyon."""
    import yfinance as yf
    # Borsa İstanbul kodları için sonuna .IS ekliyoruz
    kod = hisse_kodu.upper().strip()
    if not kod.endswith(".IS"):
        kod += ".IS"
    df = yf.download(kod, period="60d", interval="1d")
    return df

def teknik_analiz_yap(df: pd.DataFrame):
    """Teknik göstergeleri (RSI, MACD, Moving Averages vb.) hesaplar."""
    son_kapanis = float(df['Close'].iloc[-1])
    ortalamanin_ustunde = son_kapanis > float(df['Close'].mean())
    sinyal = "GÜÇLÜ AL" if ortalamanin_ustunde else "NÖTR / SAT"
    
    return f"Son Kapanış: {son_kapanis:.2f} TL | Sinyal: {sinyal}"

def lstm_tahmin_yap(df: pd.DataFrame):
    """
    Eğitilmiş LSTM modelini çalıştırıp ertesi günün tahmini kapanış fiyatını döndürür.
    """
    son_fiyat = float(df['Close'].iloc[-1])
    # Burada daha önce yazdığımız LSTM modelinin predict çıktısını döndürüyorsun.
    # Örnek olarak model çıktısını simüle edelim:
    tahmin_fiyati = son_fiyat * 1.03  # %3 yükseliş tahmini
    return round(tahmin_fiyati, 2)
...
# Pylance 2182. satır hatasını çözen eksik değişken:
sadece_super_sinyal = False


# -------------------------------------------------------------------
# 3. TAIPY CALLBACK (Butona Basıldığında Çalışacak Fonksiyon)
# -------------------------------------------------------------------
def analiz_baslat(state: State):
    """
    Taipy butona basıldığında bu fonksiyonu 'state' parametresi ile çağırır.
    """
    try:
        # 1. Yükleniyor durumunu aç ve kullanıcıya bilgi ver
        state.yukleniyor = True
        state.durum_mesaji = f"{state.secili_hisse} verileri çekiliyor ve yapay zeka modeli çalıştırılıyor..."

        # 2. Veriyi çek
        df = veri_getir(state.secili_hisse)

        if df.empty:
            state.durum_mesaji = "Hata: Hisse verisi bulunamadı veya kod hatalı!"
            state.yukleniyor = False
            return

        # 3. Analiz ve LSTM Tahmin modellerini çalıştır
        ozet_analiz = teknik_analiz_yap(df)
        tahmin_fiyat = lstm_tahmin_yap(df)

        # 4. Arayüz değişkenlerini (State) Güncelle
        # Taipy bu değerler değiştiği an ekrandaki alanları otomatik yeniler!
        state.analiz_sonucu = ozet_analiz
        state.ai_hedef = tahmin_fiyat
        state.durum_mesaji = "Analiz ve AI tahmini başarıyla tamamlandı!"

    except Exception as e:
        state.durum_mesaji = f"Analiz sırasında bir hata oluştu: {str(e)}"

    finally:
        # İşlem bitince yükleniyor modunu kapat
        state.yukleniyor = False


# -------------------------------------------------------------------
# 4. TAIPY ARAYÜZ (GUI) TASARIMI
# -------------------------------------------------------------------
page = """
# 📈 Borsa İstanbul AI Analiz & Gün İçi Tahmin Paneli
...
<|Analizi Başlat|button|on_action=analiz_baslat|active={not yukleniyor}|>
|>
...
<|
### 🤖 AI Ertesi Gün Hedef Fiyat
**Tahmini Kapanış:** <|{ai_hedef}|text|> TL
|>
|>
"""
